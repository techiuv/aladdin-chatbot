  ...
  const handlePasswordSubmit = async (data) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_SERVER_URL}/admin/validate-password`,
        {
    ...
    <>
      ...
      <main className="p-6 w-screen min-h-screen flex justify-center items-center bg-dark flex-col text-white">
        ...
        {!isAuthenticated ? (
          ...
          >
            <p className="mb-4 text-textlight fomt-lg w-3/4 mx-auto text-center">
              Please enter the admin password to access the data
            </p>
              ...
              <table className="w-full table-auto border-collapse">
                <thead>
                  <tr className="bg-tertiary">
                    <th className="py-2 px-4 text-left text-sm font-medium text-white">
                      Name
                    </th>
                    <th className="py-2 px-4 text-left text-sm font-medium text-white">
                      Email
                ...
                <tbody>
                  {users.map((user) => (
                    <tr
                      key={user._id}
                      className="border border-tertiary text-textlight"
                    >
                      <td className="py-2 px-4 text-sm text-textlight">
                        {user.name}
                      </td>
                      <td className="py-2 px-4 text-sm text-textlight">
                        {user.email}
